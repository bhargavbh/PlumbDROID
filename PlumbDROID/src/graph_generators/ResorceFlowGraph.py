...
def minimize_vfg(rfg,node_map):
    for block in rfg.block:
        node = node_map[block]
...
def create_vfg_nodes(block):
    ...
    for ins in block.ins:
        if isSourceNode(ins,resource1.request):
            vfg_nodelist.append("SourceNode:"+isSourceNode(ins,resource1.request))
        elif isFreeNode(ins,resource1.release):
            vfg_nodelist.append("FreeNode:"+isFreeNode(ins,resource1.release))
        elif isCallNode(ins):
            vfg_nodelist.append("CallNode")
        elif isExitNode(ins):
            vfg_nodelist.append("ExitNode")
    if len(vfg_nodelist)==0:
        vfg_nodelist.append("NopNode")
    #else:
...
'''for functionX in functionList:
    print functionX.label
    for x in functionX.block:
        print "label",x.label
        print "name",x.name
        print "link",x.nexts
        for ins in x.ins:
